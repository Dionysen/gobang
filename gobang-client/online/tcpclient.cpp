...
#ifdef WIN32
    servaddr.sin_addr.s_addr = inet_addr(m_IPAddress.c_str());
#else
    inet_pton(AF_INET, m_IPAddress.c_str(), &servaddr.sin_addr);
#endif
...
void tcpclient::parseInformation(onlinegame& OnlineGame, lobby& Lobby, std::string buff)
{
    ...
    else if (head.compare("board") == 0)
    {  // Update board
        std::cout << "Client: " << json["connfd"].i32() << " Update board: " << buff << std::endl;
        ...
        {
            if (OnlineGame.selfColor == BLACK_CHESS)
            {                                           // if self is black
    ...
    else if (head.compare("room") == 0)
    {  // Update room info
        std::cout << "Client: " << json["connfd"].i32() << " Read room info: " << buff << std::endl;
    ...
    else if (head.compare("user") == 0)
    {  // Update User Info
        std::cout << "Client:" << json["connfd"].i32() << " Read user info: " << buff << std::endl;
    ...
    {
        OnlineGame.setCurrentChess(chess(json["x"].i32(), json["y"].i32(), json["color"].i32(), json["step"].i32()));
        OnlineGame.update();
    }
