...
bool tcpserver::initServer(const unsigned int port)
{
    ...
    int nagleStatus = 1;
    int result      = setsockopt(m_sockfd, IPPROTO_TCP, TCP_NODELAY, (char*)&nagleStatus, sizeof(int));
    if (result == -1)
    {
        throw "Failed to change nagle status\n";
    }
#ifdef _WIN32
    ZeroMemory(&m_servaddr, sizeof(m_servaddr));  // Zero the socket address
#else
    bzero(&m_servaddr, sizeof(m_servaddr));  // Zero the socket fd
#endif

    m_servaddr.sin_family      = AF_INET;              // IPv4
    m_servaddr.sin_addr.s_addr = htonl(INADDR_ANY);    // Any IP address can connect
    m_servaddr.sin_port        = htons(DEFAULT_PORT);  // Set the port of server address struct
    if (-1 == bind(m_sockfd, (struct sockaddr*)&m_servaddr, sizeof(m_servaddr)))
    {
